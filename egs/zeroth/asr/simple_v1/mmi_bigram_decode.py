...
def main():
    ...
    logging.debug(f'loading model elapsed: {end - start}')

    assert P.requires_grad is False
    P.scores = model.P_scores.cpu()
    print_transition_probabilities(
        P, lexicon.phones, phone_ids, filename='model_P_scores.txt')

    P.set_scores_stochastic_(model.P_scores)
    print_transition_probabilities(
        P, lexicon.phones, phone_ids, filename='P_scores.txt')
    ...
    if not os.path.exists(lang_dir / 'HLG.pt'):
        ...
        logging.debug(f'Loading L_disambig.fst.txt elapsed: {end - start}')
        ...
        logging.debug(f'Loading G.fst.txt elapsed: {end - start}')
        ...
        logging.debug(f'Compiling HLG elapsed: {end - start}')
    ...
    cuts_test = CutSet.from_json(feature_dir / 'cuts_testData02.json.gz')
    ...
    logging.debug(f'decoding elapsed: {end - start}')
    ...
    logging.debug(f'WER computation elapsed: {end - start}')
