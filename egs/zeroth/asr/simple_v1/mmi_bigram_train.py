...
def get_objf(batch: Dict,
    ...
    supervision_segments, texts = encode_supervisions(supervisions, 3)
    ...
    with grad_context():
        ...
        nnet_output = nnet_output.permute(
            0, 2, 1)  # now nnet_output is [N, T, C]
        mmi_loss, tot_frames, all_frames = loss_fn(
            nnet_output, texts, supervision_segments)
...
def main():
    ...
    cuts_train = (
        load_manifest(feature_dir / 'cuts_recData01.json.gz') +
        load_manifest(feature_dir / 'cuts_recData02.json.gz') +
        load_manifest(feature_dir / 'cuts_recData03.json.gz')
    )
    # cuts_train = CutSet.from_json(feature_dir /
    #                              'cuts_train-clean-100.json.gz')
    logging.info("About to get dev cuts")
    cuts_dev = (
        load_manifest(feature_dir / 'cuts_testData01.json.gz')
    )
    ...
    valid_sampler = SingleCutSampler(
        cuts_dev, max_frames=90000, world_size=1, rank=0)
    ...
    if start_epoch > 0:
        model_path = os.path.join(
            exp_dir, 'epoch-{}.pt'.format(start_epoch - 1))
        ckpt = load_checkpoint(
            filename=model_path, model=model, optimizer=optimizer, scheduler=lr_scheduler)
        best_objf = ckpt['objf']
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if tb_writer is not None:
            tb_writer.add_scalar('train/learning_rate',
                                 curr_learning_rate, global_batch_idx_train)
        ...
        epoch_info_filename = os.path.join(
            exp_dir, 'epoch-{}-info'.format(epoch))
